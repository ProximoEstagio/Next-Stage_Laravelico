/**
 * progresso.js — Barra de progresso dinâmica (API Laravel)
 */
async function carregarProgresso() {
  ...
  try {
    ...
    const tipos = await Api.get("/aluno/tipos");
    const barra = document.getElementById("barra-progresso");

    if (barra && tipos?.length) {
      barra.innerHTML = tipos
        .map((tipo, i) => {
          const completo = tiposEnviados.includes(tipo.nome);
          const circleClass = completo ? "circleComplete" : "circle";
          const progWidth = completo ? "100%" : "0%";
          const isLast = i === tipos.length - 1;

          return `
                  <div id="circle-${tipo.nome}" class="${circleClass}"><p>${
            tipo.nome
          }</p></div>
                  ${
                    !isLast
    ...
    const caminho = ultimo.caminho_arquivo
      ? `${window.BASE}/storage/${ultimo.caminho_arquivo}`
      : null;
                ...
                .map(
                  ...
                  <div class="rw g8">
                      ...
                      <div class="cl g4">
                          ...
                          <p style="color:${
                            n.vencido ? "var(--off)" : "var(--clock)"
                          }">
                              ${n.vencido ? "Prazo vencido em " : "Vence em "}
                              ${new Date(
                                n.prazoFinal + "T00:00:00"
              ...
              ${prazos
                .map((p) => {
                  const prazoFmt = p.prazoFinal
                    ? new Date(p.prazoFinal + "T00:00:00").toLocaleDateString(
                        "pt-BR"
                      )
                    : "Não definido";
                  const cor = p.vencido
                  ...
                  return `
                      <div class="rw g8">
                          ...
                          <div class="cl g4">
                              ...
                              ${
                                p.jaEnviou
                                  ? '<p style="color:var(--check);font-size:12px">✓ Já enviado</p>'
                                  : ""
                              }
                          </div>
                      </div>`;
