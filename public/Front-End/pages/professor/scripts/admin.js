...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("click", criarTipo);
  document
    .getElementById("btn-novo-professor")
    .addEventListener("click", abrirPopupProfessor);
  document
    .getElementById("btn-novo-curso")
    .addEventListener("click", abrirPopupCurso);
});
...
async function carregarTipos() {
  ...
  const [data, dataModelos] = await Promise.all([
    Api.get("/admin/tipos"),
    Api.get("/professor/modelos").catch(() => null),
  ]);

  if (!data || !data.length) {
    lista.innerHTML = "<p>Nenhum tipo cadastrado.</p>";
    return;
  }
  ...
  lista.innerHTML = data
    .map((tipo) => {
      ...
      return `
      <div class="rw g16 jc-sb" style="padding:4px 0; flex-wrap:wrap;">
          ...
          <div class="rw g8 fc">
              ...
              ${
                temModelo
                  ? `
              <button class="btn-link fc" onclick="removerModelo('${tipo.nome}')"
                  style="color:var(--off);border-color:var(--off)">
                  Remover Modelo
              </button>`
                  : ""
              }
...
async function toggleTipo(tipoId) {
  const data = await Api.post("/admin/tipos", {
    action: "toggleAtivo",
    tipo_id: tipoId,
  });
...
async function removerModelo(tipoNome) {
  const confirmar = confirm(
    "Remover o modelo do Documento " +
      tipoNome +
      " da exibição?\n\n" +
      "O arquivo continua salvo no sistema por segurança."
  );
  if (!confirmar) return;

  const data = await Api.post("/professor/modelos/remover", { tipo: tipoNome });
  if (data?.ok) {
    carregarTipos();
  } else {
    ...
    .map(
      ...
      <div class="rw g16 jc-sb">
          <div class="cl g4">
              ...
              <p>${p.email}</p>
              <p>${
                p.nomeCurso ? "Curso: " + p.nomeCurso : "Sem curso vinculado"
              }</p>
          </div>
      </div>`
    )
...
async function abrirPopupProfessor() {
  ...
  document
    .getElementById("btn-salvar-professor")
    .addEventListener("click", salvarProfessor);
}
      ...
      <div class="container cl" style="border:none;box-shadow:none;">
          <div class="rw g16 jc-sb">
              <div class="cl g4">
                  ...
                  <p>${
                    c.nomeProfessor
                      ? "Professor: " + c.nomeProfessor
                      : "Sem professor vinculado"
                  }</p>
              </div>
              <button class="btn-link fc" onclick="abrirPopupReatribuir(${
                c.idcurso
              }, '${c.nomeCurso}')">
...
async function abrirPopupCurso() {
  ...
  popup.innerHTML = `
      <div class="popup slim container">
          ...
          <div class="cl p16 g16">
              ...
              ${
                !semCurso.length
                  ? '<p style="color:var(--vermelho);font-size:12px">Todos os professores já estão vinculados.</p>'
                  : ""
              }
  ...
  document.getElementById("close-popup").addEventListener("click", fecharPopup);
  document
    .getElementById("btn-salvar-curso")
...
async function salvarCurso() {
  ...
  const data = await Api.post("/admin/cursos", {
    nomeCurso,
...
async function abrirPopupReatribuir(cursoId, nomeCurso) {
  ...
  document.getElementById("close-popup").addEventListener("click", fecharPopup);
  document
    .getElementById("btn-salvar-reatribuir")
    .addEventListener("click", async () => {
      ...
      const data = await Api.post("/admin/cursos/atualizar", {
        curso_id: cursoId,
        professor_id: professorId,
      });
      if (data?.ok) {
