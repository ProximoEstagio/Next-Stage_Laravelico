 ...
 * Ícones de status clicáveis → abre tela do aluno
 */
...
function criarSkeletons(qtd) {
  ...
  for (let i = 0; i < qtd; i++) {
    ...
    line.innerHTML = `
            <td class="loading"></td>
            <td class="loading"></td>
            <td><div class="rw jc-sa">${tiposAtivos
              .map(() => '<span class="icon-list empty"></span>')
              .join("")}</div></td>
...
function atualizarCabecalho() {
  const thead = document.querySelector("table.container.topC thead tr");
  if (!thead) return;

  const icones = tiposAtivos.map((t) => `<p>${t.nome}</p>`).join("");
  thead.innerHTML = `
        <th><p>Nome</p></th>
        <th><p>R.A. do Aluno</p></th>
        <th><div class="rw jc-sa">${icones}</div></th>
        <th></th>`;
}
...
function renderizarLista(alunos, pagina) {
  ...
  alunosPagina.forEach((user) => {
    // Ícones clicáveis — cada um abre a tela do aluno
    const icones = tiposAtivos
      .map((t) => {
        const status = user["status_" + t.nome] || "empty";
        return `
                <span class="icon-list ${getIconClass(status)}"
                    title="Documento ${t.nome}: ${status}"
                    onclick="window.open('aluno.html?id=${
                      user.idaluno
                    }', '_blank')"
...
function aplicarFiltros() {
  ...
  const email =
    document
      ...
      .trim() || "";
  const semestre =
    document.querySelector('input[placeholder="Semestre"]')?.value.trim() || "";
...
document.addEventListener("DOMContentLoaded", async () => {
  const tipos = await Api.get("/professor/tipos");
